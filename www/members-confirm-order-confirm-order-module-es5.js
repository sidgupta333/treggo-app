...
var ConfirmOrderPage = /** @class */ (function () {
    function ConfirmOrderPage(rest, loading, storage, dialogs, router) {
        this.rest = rest;
    ...
    ConfirmOrderPage.prototype.ngOnInit = function () {
        ...
        this.loading.create({
            ...
            _this.storage.ready().then(function () {
                _this.storage.get('dishes').then(function (res) {
                    loading.dismiss();
                    res.forEach(function (category) {
                        category.dishes.forEach(function (dish) {
                            if (dish.count > 0) {
                                _this.orderItems.items.push({
                                ...
                                _this.total += (dish.base_price * dish.count);
                            }
                        });
                    });
                    _this.taxAmount = (0.12 * _this.total).toFixed(2);
                    _this.grandTotal = parseFloat(_this.total) + parseFloat(_this.taxAmount);
                });
            });
        });
    };
    ConfirmOrderPage.prototype.completeOrder = function () {
        var _this = this;
        this.loading.create({
    ...
    //Create new suborder
    ConfirmOrderPage.prototype.createSubOrder = function (status, loading) {
        var _this = this;
        var dto = {
        ...
        this.rest.createSubOrder(dto).subscribe(function (res) {
            _this.orderItems.order_id = _this.order.order_id;
            _this.orderItems.sub_order_id = res.sub_order_id;
            _this.orderItems.status = res.status;
            _this.storage.set('placedOrder', _this.orderItems);
            console.log("Order items --> ", _this.orderItems);
            loading.dismiss();
            _this.router.navigate(['menu', 'status']);
            _this.dialogs.alert("Your food has been ordered successfully", "SUCCESS").then(function () {
                _this.router.navigate(['menu', 'status']);
            });
        }, function (err) {
            _this.dialogs.alert("Your order has been failed, Please Contact Admin ", "ERROR");
        });
    };
    //Create new order
    ConfirmOrderPage.prototype.createOrder = function (loading) {
        var _this = this;
        var dto = {
            customer_id: this.user.customer_id,
        ...
        this.rest.createOrder(dto).subscribe(function (res) {
            _this.order = res;
            _this.storage.set('order', res);
    ...
    ConfirmOrderPage.prototype.prepareDishesString = function () {
        var output = {
            dishes: "",
            quantities: ""
        };
        this.orderItems.items.forEach(function (element) {
    ...
    ConfirmOrderPage.ctorParameters = function () { return [
...
//# sourceMappingURL=members-confirm-order-confirm-order-module-es5.js.map