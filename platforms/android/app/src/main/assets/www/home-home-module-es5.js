...
var HomePage = /** @class */ (function () {
    function HomePage(formBuilder, loading, dialogs, rest, storage, router) {
    ...
    HomePage.prototype.enterOTP = function () {
        var _this = this;
        ...
        this.loading.create({
            ...
            _this.rest.createNewCustomer(dto).subscribe(function (res) {
                _this.otpDisplayed = true;
                _this.user = res;
                loading.dismiss();
            }, function (err) {
                loading.dismiss();
                _this.dialogs.alert("Unable to register the customer on this device, please contact ADMIN", "ERROR");
            });
        });
    };
    HomePage.prototype.switchToPhone = function () {
        this.otpForm.patchValue({
            otp: ''
        });
        this.otpDisplayed = false;
    };
    HomePage.prototype.submitOTP = function () {
        var _this = this;
        this.loading.create({
            ...
            //Activate Customer:
            var dto = {
                custId: _this.user.customer_id,
                status: true
            };
            _this.rest.updateCustomerStatus(dto).subscribe(function (res) {
                loading.dismiss();
                _this.user.status = 'Y';
                _this.storage.set('user', _this.user);
                console.log("User storage --> ", _this.user);
                _this.storage.set('order', null);
                _this.storage.set('placedOrder', null);
                _this.storage.set('dishes', null);
                //Navigate to menu after validation:
                _this.router.navigate(['menu', 'dashboard']);
            }, function (err) {
                loading.dismiss();
                _this.storage.set('order', null);
                _this.dialogs.alert("Unable to activate the customer, please contact ADMIN", "ERROR");
            });
        });
    };
